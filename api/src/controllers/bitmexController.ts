...
export const fetch = async ({originalUrl, baseUrl, body: {data, method}, query}: Request, res: Response) => {
  try {
    const path = originalUrl.replace(`${baseUrl}/`, '');
    const body = method === 'GET' ? (query ? {filter: JSON.parse((query.filter as string) || '{}')} : undefined) : data;
